...
def parse_args():
    parser = argparse.ArgumentParser(description='Training Flow Using PINN.')
...
def calculate_loss(num_samples, domain, input_data, real_data, u, l, miu):
    ...
    net_bc_1 = net(bc1)  # z=0.00; r(0.05，0.056)
    mse_uz_1 = mse_cost_function(net_bc_1[:, 0].unsqueeze(-1), velocity_zeros_100)
    mse_us_1 = mse_cost_function(net_bc_1[:, 1].unsqueeze(-1), velocity_zeros_100)
    mse_ur_1 = mse_cost_function(net_bc_1[:, 2].unsqueeze(-1), velocity_zeros_100)

    net_bc_2 = net(bc2)  # z=0.12; r(0.05，0.056)
    mse_uz_2 = mse_cost_function(net_bc_2[:, 0].unsqueeze(-1), velocity_zeros_100)
    mse_us_2 = mse_cost_function(net_bc_2[:, 1].unsqueeze(-1), velocity_zeros_100)
    mse_ur_2 = mse_cost_function(net_bc_2[:, 2].unsqueeze(-1), velocity_zeros_100)

    net_bc_3 = net(bc3)  # z(0.0,0.12); r=0.05
    mse_uz_3 = mse_cost_function(net_bc_3[:, 0].unsqueeze(-1), velocity_zeros_500)
    mse_us_3 = mse_cost_function(net_bc_3[:, 1].unsqueeze(-1), velocity_value)
    mse_ur_3 = mse_cost_function(net_bc_3[:, 2].unsqueeze(-1), velocity_zeros_500)

    net_bc_4 = net(bc4)  # z(0.0,0.12); r=0.056
    mse_uz_4 = mse_cost_function(net_bc_4[:, 0].unsqueeze(-1), velocity_zeros_500)
    mse_us_4 = mse_cost_function(net_bc_4[:, 1].unsqueeze(-1), velocity_zeros_500)
    mse_ur_4 = mse_cost_function(net_bc_4[:, 2].unsqueeze(-1), velocity_zeros_500)

    net_bc_5 = net(bc5)  # z=0; r=0.05
    
    mse_p_5 = mse_cost_function(net_bc_5[:, 3].unsqueeze(-1), velocity_zeros_500)
    
    net_in_6 = net(input_data)
    mse_data = mse_cost_function(net_in_6, real_data)
    
    f_out1 = pde(f1, u, l, miu)
    mse_f = mse_cost_function(f_out1, eq_zeros)
...
weights = torch.ones(net.output_size - 1).to(device)

headers = ['iteration', 'total', 'bc', 'data', 'f', 'error_l2']
with open(args.loss_path+filename+"-0.0510-loss.csv", 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(headers)

for epoch in range(args.num_epochs):
    optimizer.zero_grad()
    mse_uz, mse_us, mse_ur = calculate_loss(args.num_samples, domain, input_data, real_data, args.Dimension_U,
                                                 args.Dimension_L, args.miu)
    task_losses = [mse_uz, mse_us, mse_ur]
    task_losses = torch.stack(task_losses)
    weighted_loss = torch.mul(weights, task_losses)
    loss = torch.sum(weighted_loss)
    loss.backward(retain_graph=True)
    optimizer.step()
    cosine_scheduler.step()

    wz = weights.data.cpu().numpy()[0]
    ws = weights.data.cpu().numpy()[1]
    wr = weights.data.cpu().numpy()[2]
    ...
    with torch.autograd.no_grad():
            ...
            if (val_loss) < pre_loss:
                pre_loss = val_loss
                torch.save(net.state_dict(), args.model_path+filename+"-0.0510_model")
            if (epoch+1) % 100 == 0:
                number_loss = [epoch, loss.data.item(), task_losses[0].item(),  task_losses[1].item(),  task_losses[2].item(), val_reloss.data.item()]
                with open(args.loss_path+filename+"-0.0510-loss.csv", 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile)
...
file_name = "0.0510-time-consuming.txt"
with open(args.loss_path+file_name, "w") as f:
    f.write(filename+"-0.0510-time-consuming: "+str(number)+" hours")
