...
path_model = "../model/u=0.02-decrease/grad/2024-11-20-0.0050-d.pth"
...
plt.ylabel(r"$\it{τw}$$\rm(Pa)$", fontsize=20)
plt.plot(x1, 0.001003*f[:,0].detach().numpy()*U/L, "y-", label="PINN", linewidth=3)
plt.plot(x1, d*0.001003, "r-", label="CFD", linewidth=3)
print(x1)
plt.xlim(0.0, 0.178)
...
wall_shell = np.expand_dims(d*0.001003, axis=1)
pre_wall_shell = np.expand_dims(0.001003*f[:,0].detach().numpy()*U/L, axis=1)

result = np.concatenate((x, y, wall_shell, pre_wall_shell), axis=1)
headers = ['x', 'y', 'wall_shell', 'pre_wall_shell']
with open('../result-0.02-decrease/0050_wall_shell.csv','w',newline='') as csvfile:
