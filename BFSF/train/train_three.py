...
from tools.relative_error import REL2
...
def parse_args():
    parser = argparse.ArgumentParser(description='Training Backward-facing step Flow Using GB-PINN.')
    parser.add_argument('--num_epochs', type=int, default=40001, help='Number of epochs to train )')
    ...
    parser.add_argument('--Dimension_U', type=float, default=0.02, help='Dimensionless processing parameters ')
    parser.add_argument('--Dimension_L', type=float, default=0.01, help='Dimensionless processing parameters ')
    parser.add_argument('--weight_eq', type=float, default=1, help='weight_eq ')
    parser.add_argument('--weight_bc', type=float, default=1, help='weight_bc ')
    parser.add_argument('--weight_data', type=float, default=1, help='weight_data ')
    parser.add_argument('--Reynolds_number', type=float, default=200, help='Re ')
    parser.add_argument('--data_path', type=str, default='../data/dataset/U=0.02-decrease/', help='Path to the data ')
    parser.add_argument('--loss_path', type=str, default='../data/loss/U=0.02-decrease/grad/', help='Path to the data ')
    parser.add_argument('--model_path', type=str, default='../model/u=0.02-decrease/grad/', help='Path to save the trained model')
...
class PINN_Train(torch.nn.Module):
    def __init__(self, model):
        ...
        self.model = model
        self.weights = torch.nn.Parameter(torch.ones(model.output_size).float())
...
for epoch in range(args.num_epochs):
    ...
    with torch.autograd.no_grad():
        ...
        val_err = torch.norm( v_real_data.flatten() - net_val.flatten(), p=2)
        val_loss = val_err/torch.norm(v_real_data.flatten(), p=2)
        print(epoch, "Training Loss:", loss.data.item(), " ; Verifying Loss:", val_err.data.item(), " ; Verifying ab_err:", val_loss.data.item())
        print("   weight_bc:", wx, " ; weight_data:", wy, " ; weight_f:", wp)
        if (val_loss) < pre_loss or model_num>=500:
            pre_loss = val_err
